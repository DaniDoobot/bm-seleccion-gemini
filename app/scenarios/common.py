...
def make_save_candidate_context_handler(
    ...
    def handler(args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Set provisional candidate info
        session.provisional_name = name_str
        session.provisional_lastname = lastname_str

        # Commit context using the centralized helper
        from unittest.mock import MagicMock
        if hasattr(session, "commit_candidate_context") and not isinstance(session.commit_candidate_context, MagicMock):
            res = session.commit_candidate_context()
        else:
            ...
            res = {"status": "saved", "success": True}

        if res.get("status") == "already_saved":
            logger.info("save_candidate_context: Duplicate execution detected during commit.")
            return {
                "success": False,
                "error": "already_saved",
                "status": "already_saved",
        ...
        logger.info(
            ...
            masked_name, masked_lastname, rgpd_bool, scenario
        )
