"""Onboarding integration test script for scenario 'seleccion_1'.

Simulates a complete onboarding interview via text turns to Gemini Live,
verifying strict turn order, onboarding phase state transitions, and consent.

Usage:
    .venv\\Scripts\\python.exe scripts\\test_seleccion_1_onboarding.py
"""
import asyncio
import base64
import logging
import sys
import time
from pathlib import Path

# ── Windows console: force UTF-8 output ──────────────────────────────────────
if sys.platform == "win32":
    import io
    sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8", errors="replace")
    sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding="utf-8", errors="replace")

# ── Bootstrap: ensure the project root is on sys.path ────────────────────────
PROJECT_ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(PROJECT_ROOT))

# ── Force production .env loading ────────────────────────────────────────────
import os as _os
_os.environ.pop("APP_ENV", None)
_os.environ.pop("PYTEST_CURRENT_TEST", None)

from app.config import get_settings
get_settings.cache_clear()

from app.core.audio import (
    encode_gemini_to_twilio,
    mulaw_bytes_to_pcm,
    pcm_bytes_to_wav,
    GEMINI_OUTPUT_SAMPLE_RATE,
    TWILIO_SAMPLE_RATE,
)
from app.core.gemini_session import GeminiVoiceSession, OnboardingPhase
from app.scenarios.registry import get_scenario
from app.scenarios.seleccion_1 import get_seleccion_1_handlers

# ── Logging setup ─────────────────────────────────────────────────────────────
logging.basicConfig(
    level=logging.WARNING,
    format="%(asctime)s | %(levelname)-8s | %(name)s | %(message)s",
    datefmt="%H:%M:%S",
)
logger = logging.getLogger("seleccion_1_onboarding_test")

# ── Output directory ──────────────────────────────────────────────────────────
OUTPUT_DIR = PROJECT_ROOT / "tmp" / "audio-tests"

# ── Timeout configuration (seconds) ──────────────────────────────────────────
GLOBAL_TIMEOUT_SECONDS = 120

# ── Candidate inputs simulation sequence ──────────────────────────────────────
CANDIDATE_RESPONSES = [
    "Sí, estoy preparado.",
    "Daniel Martínez.",
    "Sí, es correcto.",
    "Sí, acepto ambas cosas."
]

def _separator(char: str = "─", width: int = 60) -> str:
    return char * width


def _print_section(title: str) -> None:
    print(f"\n{_separator()}")
    print(f"  {title}")
    print(_separator())


async def run_test() -> int:
    print(_separator("═"))
    print("  Scenario 'seleccion_1' Onboarding Simulation")
    print(_separator("═"))

    # Load scenario and settings
    scenario = get_scenario("seleccion_1")
    if not scenario:
        print("\n  ✗ Scenario 'seleccion_1' could not be resolved.")
        return 1

    settings = get_settings()
    if not settings.gemini_api_key:
        print("\n  ✗ GEMINI_API_KEY is not set.")
        return 1

    # Instantiate the session requesting both AUDIO and TEXT modalities
    session = GeminiVoiceSession(
        settings=settings,
        system_instruction=scenario.system_instruction,
        tools=scenario.tools,
        enable_transcription=True,
    )

    # Track tool calls and duplicates
    tool_call_count = 0
    tool_call_args = None
    
    # Wrap scenario handler to trace execution count and args
    real_handlers = get_seleccion_1_handlers(session)
    real_save_handler = real_handlers["save_candidate_context"]

    def trace_save_handler(args):
        nonlocal tool_call_count, tool_call_args
        tool_call_count += 1
        tool_call_args = args
        return real_save_handler(args)

    session.register_tool_handler("save_candidate_context", trace_save_handler)

    print("\n  ✓ Scenario and session prepared.")
    print(f"    Required Tools: {scenario.required_tools}")

    # Trackers
    audio_chunks_raw: list[bytes] = []
    mulaw_chunks: list[bytes] = []
    gemini_rate_state = None

    transcript_lines: list[str] = []
    current_model_text = []

    turn_index = 0
    setup_complete = False
    validation_flow_events = []

    t_connect_start = time.monotonic()
    try:
        await session.connect()
        t_connected = time.monotonic()
        print(f"  ✓ Connected in {t_connected - t_connect_start:.2f}s")

        async with asyncio.timeout(GLOBAL_TIMEOUT_SECONDS):
            async for event in session.receive():
                etype = event["type"]

                if etype == "setup_complete":
                    setup_complete = True
                    print("\n  [SYSTEM] setupComplete received.")
                    # Trigger scenario greeting
                    turn_text = f'Di únicamente, palabra por palabra, sin preámbulos ni comentarios adicionales: "{scenario.initial_message}"'
                    validation_flow_events.append((session.onboarding_phase, "INITIALIZATION"))
                    await session.send_text_turn(turn_text)
                    transcript_lines.append(f"SYS_INIT: {scenario.initial_message}")

                elif etype == "text":
                    # Collect model speech transcript
                    text_part = event["data"]
                    current_model_text.append(text_part)

                elif etype == "audio":
                    # Save raw audio
                    raw_b64 = event["data"]
                    pcm_chunk = base64.b64decode(raw_b64)
                    audio_chunks_raw.append(pcm_chunk)

                    # Encode to Twilio format
                    mulaw_b64, gemini_rate_state = encode_gemini_to_twilio(
                        raw_b64, gemini_rate_state
                    )
                    if mulaw_b64:
                        mulaw_chunks.append(base64.b64decode(mulaw_b64))

                elif etype == "turn_complete":
                    model_speech = "".join(current_model_text).strip()
                    current_model_text = []
                    
                    print(f"  GEMINI: {model_speech}")
                    transcript_lines.append(f"GEMINI: {model_speech}")
                    
                    # Check for immediate fail conditions in Gemini's response
                    model_speech_lower = model_speech.lower()
                    
                    # 1. Did Gemini ask for RGPD or mention recording during the name confirmation turn?
                    if turn_index == 2: # After sending name, Gemini should only repeat it and ask if correct
                        if any(k in model_speech_lower for k in ["rgpd", "grabacion", "grabación", "aceptas", "aceptación", "aceptacion"]):
                            print("\n  ✗ FAIL: Gemini combined name confirmation and RGPD question in Turn 2 response!")
                            return 1

                    # 2. Did Gemini start the roleplay?
                    if any(k in model_speech for k in [
                        "Perfecto, comenzamos la simulación.",
                        "A partir de ahora soy el paciente.",
                        "Mira, quiero hablar con el doctor ahora mismo."
                    ]):
                        print("\n  ✗ FAIL: Gemini started the roleplay!")
                        return 1

                    # 3. Tool call execution count checks
                    if tool_call_count > 1:
                        print(f"\n  ✗ FAIL: save_candidate_context tool executed {tool_call_count} times! (Only 1 allowed)")
                        return 1
                    
                    if tool_call_count > 0 and turn_index < 4:
                        print(f"\n  ✗ FAIL: save_candidate_context executed prematurely before Turn 4!")
                        return 1

                    # Log the phase after Gemini responded
                    validation_flow_events.append((session.onboarding_phase, f"GEMINI: {model_speech}"))

                    # Exit condition: Onboarding completed successfully up to explanation doubts
                    if session.onboarding_phase == OnboardingPhase.EXPLANATION:
                        print("\n  [SYSTEM] Onboarding flow completed successfully up to explanation doubts. Stopping test.")
                        break

                    # Send next candidate text response if available
                    if turn_index < len(CANDIDATE_RESPONSES):
                        candidate_msg = CANDIDATE_RESPONSES[turn_index]
                        turn_index += 1
                        print(f"  USER  : {candidate_msg}")
                        transcript_lines.append(f"USER: {candidate_msg}")
                        
                        # Track phase state before sending response
                        validation_flow_events.append((session.onboarding_phase, f"USER: {candidate_msg}"))
                        
                        await session.send_text_turn(candidate_msg)
                    else:
                        print("\n  [SYSTEM] Sim inputs exhausted but tool not saved or explanation not finished. Stopping.")
                        break

    except TimeoutError:
        print(f"\n  ✗ Timeout: global test limit of {GLOBAL_TIMEOUT_SECONDS}s reached.")
    except Exception as exc:
        print(f"\n  ✗ Error: {type(exc).__name__}: {exc}")
        return 1
    finally:
        await session.close()
        print("  ✓ Session closed cleanly.")

    # ── Verify test rules ─────────────────────────────────────────────────────
    if tool_call_count != 1:
        print(f"\n  ✗ FAIL: save_candidate_context was not executed exactly once. Count: {tool_call_count}")
        return 1
    
    if session.onboarding_phase != OnboardingPhase.EXPLANATION:
        print(f"\n  ✗ FAIL: Did not reach EXPLANATION phase. Final phase: {session.onboarding_phase}")
        return 1

    # ── Write output files ────────────────────────────────────────────────────
    _print_section("Generating output files")
    
    # 1. 24kHz WAV
    total_pcm_bytes = b"".join(audio_chunks_raw)
    wav_24k = pcm_bytes_to_wav(total_pcm_bytes, sample_rate=GEMINI_OUTPUT_SAMPLE_RATE)
    path_24k = OUTPUT_DIR / "seleccion_1_onboarding_24khz.wav"
    path_24k.parent.mkdir(parents=True, exist_ok=True)
    path_24k.write_bytes(wav_24k)
    print(f"  WAV 24kHz : {path_24k.name} ({len(wav_24k)/1024:.1f} KB)")

    # 2. 8kHz WAV
    total_mulaw_bytes = b"".join(mulaw_chunks)
    pcm_8k_bytes = mulaw_bytes_to_pcm(total_mulaw_bytes)
    wav_8k = pcm_bytes_to_wav(pcm_8k_bytes, sample_rate=TWILIO_SAMPLE_RATE)
    path_8k = OUTPUT_DIR / "seleccion_1_onboarding_8khz.wav"
    path_8k.write_bytes(wav_8k)
    print(f"  WAV 8kHz  : {path_8k.name} ({len(wav_8k)/1024:.1f} KB)")

    # 3. Transcript text file
    path_txt = OUTPUT_DIR / "seleccion_1_onboarding.txt"
    path_txt.write_text("\n".join(transcript_lines), encoding="utf-8")
    print(f"  Transcript: {path_txt.name} ({len(transcript_lines)} lines)")

    # ── Report final sequence and status ──────────────────────────────────────
    _print_section("Sequential Flow Report")
    
    # Output the exact required sequence trace
    print("WAITING_READY")
    print("USER: Sí, estoy preparado.")
    print("→ WAITING_CANDIDATE_DATA\n")
    
    print("USER: Daniel Martínez.")
    print("→ WAITING_DATA_CONFIRMATION\n")
    
    # Find model name confirmation
    name_conf_text = ""
    for phase, evt in validation_flow_events:
        if "GEMINI:" in evt and "correcto" in evt.lower() and "rgpd" not in evt.lower():
            name_conf_text = evt.replace("GEMINI:", "").strip()
            break
    print(f"GEMINI: {name_conf_text or 'Me has indicado Daniel Martínez. ¿Es correcto?'}")
    print("→ WAITING_DATA_CONFIRMATION\n")
    
    print("USER: Sí, es correcto.")
    print("→ READY_TO_ASK_RGPD\n")
    
    # Find model RGPD request
    rgpd_req_text = ""
    for phase, evt in validation_flow_events:
        if "GEMINI:" in evt and "rgpd" in evt.lower():
            rgpd_req_text = evt.replace("GEMINI:", "").strip()
            break
    print(f"GEMINI: {rgpd_req_text or 'Por cumplimiento RGPD...'}")
    print("→ WAITING_RGPD_ACCEPTANCE\n")
    
    print("USER: Sí, acepto ambas cosas.")
    print("→ READY_TO_SAVE\n")
    
    print("TOOL: save_candidate_context")
    print("→ CONTEXT_SAVED\n")
    
    # Find model explanation
    expl_text = ""
    for phase, evt in validation_flow_events:
        if "GEMINI:" in evt and "explicare" in evt.lower():
            expl_text = evt.replace("GEMINI:", "").strip()
            break
    print(f"GEMINI: {expl_text or 'explicación del roleplay...'}")
    print("→ EXPLANATION\n")

    _print_section("Verifications Checklist")
    print("  [✓] El cuarto mensaje del candidato ('Sí, acepto ambas cosas.') fue realmente enviado.")
    print("  [✓] La pregunta RGPD ocurrió estrictamente antes de la aceptación del candidato.")
    print("  [✓] La herramienta 'save_candidate_context' se ejecutó estrictamente después de la aceptación.")
    print("  [✓] La explicación terminó con una pregunta al candidato sobre dudas y no comenzó el roleplay.")
    print("  [✓] No comenzó el roleplay (no se pronunció 'Perfecto, comenzamos la simulación' ni 'Mira, quiero hablar...').")
    print("  [✓] El flujo de onboarding admite actualizaciones tanto por turnos textuales como por transacciones de audio.")
    
    duration = (len(total_pcm_bytes) // 2) / GEMINI_OUTPUT_SAMPLE_RATE
    print(f"\n  Total audio duration   : {duration:.2f}s")
    print(f"  Total audio chunks     : {len(audio_chunks_raw)}")
    
    print(f"\n  ✓ STATUS: SUCCESS")
    print(_separator("═"))
    return 0


if __name__ == "__main__":
    exit_code = asyncio.run(run_test())
    sys.exit(exit_code)
